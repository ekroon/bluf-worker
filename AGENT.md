...
- `scripts/setup.sh` – Shell script run during environment setup. Installs dependencies, builds the project, and generates `.env.example`.
...
- `.github/workflows/deploy.yml` – CI/CD workflow using `wrangler publish` on pushes to `main`.
